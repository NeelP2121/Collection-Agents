...
class BorrowerContext:
    ...
    def update_from_handoff(self, handoff_summary: Dict[str, Any]):
        ...
        if "identity_verified" in handoff_summary:
            self.identity_verified = handoff_summary["identity_verified"]
        
        # Update hardship status
        if "hardship_status" in handoff_summary:
            self.hardship_detected = handoff_summary.get("hardship_status", "").lower() in ["yes", "true", "detected"]
        
        # Update financial information if provided
        if "financial_summary" in handoff_summary:
            # Could parse financial details from summary, but for now just store
            pass
            
        # Update borrower profile/personality assessment
        if "borrower_profile" in handoff_summary:
            # Store personality assessment for future agents
            pass
            
        # Update settlement interest/offers
        if "settlement_offers" in handoff_summary:
            self.agent2_offers_made = handoff_summary["settlement_offers"]
            
        # Update communication preferences
        if "communication_notes" in handoff_summary:
